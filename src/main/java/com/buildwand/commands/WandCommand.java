...
public class WandCommand {
    ...
    private static int executeWandCommand(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        lore.add(new StringNBT("{\"text\":\"İlk köşeyi seçmek için sol tıklayın\",\"color\":\"gray\"}"));
        lore.add(new StringNBT("{\"text\":\"İkinci köşeyi seçmek için sağ tıklayın\",\"color\":\"gray\"}"));
        lore.add(new StringNBT("{\"text\":\"Seçimden sonra malzemeyi ve miktarı belirtin\",\"color\":\"yellow\"}"));
        ...
        if (!player.getHeldItem(Hand.MAIN_HAND).isEmpty()) {
            ...
            if (!player.inventory.addItemStackToInventory(wandItem)) {
                player.sendMessage(new StringTextComponent(TextFormatting.RED + "Seçim Baltası size veremedim. Envanteriniz dolu."));
                return 0;
            } else {
                player.sendMessage(new StringTextComponent(TextFormatting.GREEN + "Seçim Baltası envanterinize eklendi."));
            }
        } else {
            // Place in main hand
            player.setHeldItem(Hand.MAIN_HAND, wandItem);
            player.sendMessage(new StringTextComponent(TextFormatting.GREEN + "Elinize seçim baltası verildi."));
        }
    ...
    private static int executeUndo(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        // If no recent build, try to clear selection
            ...
            if (selection != null && (selection.getFirstPosition() != null || selection.getSecondPosition() != null)) {
                ...
                player.sendMessage(new StringTextComponent(TextFormatting.YELLOW + "Alan seçiminiz temizlendi."));
            } else {
                player.sendMessage(new StringTextComponent(TextFormatting.RED + "Geri alınacak yeni bir yapı veya temizlenecek bir seçim yok."));
            }
        }
        return 0;
    }
    ...
    private static int executeClearSelection(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        if (selection != null && (selection.getFirstPosition() != null || selection.getSecondPosition() != null)) {
            // Clear the selection
            BuildWandPlugin.getInstance().getPlayerSelections().remove(playerUUID);
            player.sendMessage(new StringTextComponent(TextFormatting.YELLOW + "Your area selection has been cleared."));
        } else {
            player.sendMessage(new StringTextComponent(TextFormatting.RED + "You don't have an active selection to clear."));
        }
        return 1;
    }
    ...
    private static int showHelp(CommandContext<CommandSource> context) throws CommandSyntaxException {
        PlayerEntity player = context.getSource().asPlayer();
        player.sendMessage(new StringTextComponent(TextFormatting.GOLD + "=== BuildWand Commands ==="));
        player.sendMessage(new StringTextComponent(
